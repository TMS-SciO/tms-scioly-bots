...
class RoboPages(discord.ui.View):
    ...
    async def start(self) -> None:
        if self.check_embeds and not self._interaction.channel.permissions_for(self._interaction.guild.me).embed_links:
            try:
        ...
        self._update_labels(0)
        try:
            self.message = await self._interaction.response.send_message(**kwargs, view=self)
        except discord.InteractionResponded:
