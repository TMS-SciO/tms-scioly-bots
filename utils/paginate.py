...
class RoboPages(discord.ui.View):
    def __init__(
        ...
        *,
        ...
        self.source: menus.PageSource = source
        self.check_embeds: bool = check_embeds
        self._interaction: discord.Interaction = interaction_
        self.message: Optional[discord.Message] = None
        self.bot = bot
        self.current_page: int = 0
        self.compact: bool = compact
    ...
    async def on_error(
        ...
        if interaction.response.is_done():
            await interaction.followup.send(
                f"An unknown error occurred, sorry {error}", ephemeral=True
            )
            print(error)
        else:
            await interaction.response.send_message(
    ...
    async def start(self) -> None:
        if (
            self.check_embeds
            and not self._interaction.channel.permissions_for(
                self._interaction.guild.me
            ).embed_links
        ):
            try:
        ...
        self._update_labels(0)
        try:
            self.message = await self._interaction.response.send_message(
                **kwargs, view=self
            )
        except discord.InteractionResponded:
    ...
    async def numbered_page(
        ...
        async with self.input_lock:
            ...
            try:
                msg = await self._interaction.client.wait_for(
                    "message", check=message_check, timeout=30.0
                )
            except asyncio.TimeoutError:
                await interaction.followup.send("Took too long.", ephemeral=True)
