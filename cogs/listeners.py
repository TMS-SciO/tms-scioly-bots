...
class Listeners(commands.Cog):
    ...
    async def on_message_edit(self, before, after):
        ...
        message_embed.add_field(
            name="Author", value=after.author.mention, inline=True
        )
        message_embed.add_field(
        ...
        message_embed.add_field(
            name="Before", value=before.content[:1024] if len(before.content) > 0 else "None", inline=True
        )
        message_embed.add_field(
            name="After", value=after.content[:1024] if len(after.content) > 0 else "None", inline=True
        )
        message_embed.add_field(
            name="Attachments",
            value=" | ".join(
        ...
        message_embed.add_field(
            name="Embeds",
            value="\n".join([str(e.to_dict()) for e in after.embeds]) if len(
                after.embeds) > 0 else "None", inline=False
        )
        message_embed.add_field(
            name="Created", value=discord.utils.format_dt(before.created_at, 'R'), inline=True
        )
        message_embed.add_field(
            name="Edited", value=discord.utils.format_dt(after.created_at, 'R'), inline=True
        )
    ...
    async def on_message_delete(self, message: discord.Message):
        ...
        if not message.guild.id == SERVER_ID:
            return
        if message.channel.type != discord.ChannelType.private and message.channel.id in (
                Channel.EDITEDM,
                Channel.DELETEDM,
                Channel.DELETEDM
        ):
        ...
        message_embed.add_field(
            name="Content", value=str(message.content)[:1024] if len(message.content) > 0 else "None", inline=False
        )
        message_embed.add_field(
            name="Attachments", value=" | ".join(
                [f"**{a.filename}**: [Link]({a.url})" for a in message.attachments]) if message.attachments else "None",
            inline=False
        )
        message_embed.add_field(
            name="Created", value=discord.utils.format_dt(message.created_at, 'R'), inline=False
        )
        message_embed.add_field(
            name="Deleted", value=discord.utils.format_dt(discord.utils.utcnow(), 'R'), inline=False
        )
        message_embed.add_field(
            name="Embed", value="\n".join([str(e.to_dict()) for e in message.embeds])[:1024] if len(
                message.embeds) > 0 else "None", inline=False
        )
