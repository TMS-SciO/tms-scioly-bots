...
class CensorGroup(Group):
    ...
    async def add(self, interaction: discord.Interaction, phrase: str):
        ...
        if phrase in CENSORED["words"]:
            ...
            await self.bot.mongo.update(
                "bot", "censor", CENSORED["_id"], {"$push": {"words": phrase}}
            )
            return await interaction.response.send_message(
                f"Added Word to censored list"
            )

    @command()
    @checks.has_any_role(Role.SERVERLEADER, Role.FORMER_SL)
    @describe(phrase="The word/phrase to remove from the censor list")
    async def remove(self, interaction: discord.Interaction, phrase: str):
        ...
        if phrase not in CENSORED["words"]:
            return await interaction.response.send_message(
                f"`{phrase}` is not in the list of censored words."
            )
        else:
            CENSORED["words"].remove(phrase)
            await self.bot.mongo.update(
                "bot", "censor", CENSORED["_id"], {"$pull": {"words": phrase}}
            )
            return await interaction.response.send_message(
                f"Removed {phrase} from list of censored words"
            )
...
class Moderation(commands.Cog):
    ...
    @staticmethod
    async def send_closed_report(
        interaction: discord.Interaction, embed: discord.Embed
    ):
        closed_reports: discord.abc.MessageableChannel = interaction.guild.get_channel(
            Channel.CLOSED_REPORTS
        )
    ...
    async def cron(self, interaction: discord.Interaction):
        ...
        if len(cron_list) == 0:
            return await interaction.response.send_message(
                "No items currently in CRON list"
            )
    ...
    async def ban(
        ...
        delete_length = {"Previous 24 hours": 1, "Previous 7 days": 7, "None": 0}
        ...
        if view.value is True:
            if member in interaction.guild.members:
                ...
                await interaction.guild.ban(
                    member, reason=reason, delete_message_days=delete_message
                )
                await interaction.edit_original_message(
                    embed=original_shown_embed, content=None
                )
                await reports_channel.send(embed=original_shown_embed)
                if ban_length != "Indefinitely":
                    cron_cog: Union[commands.Cog, CronTasks] = self.bot.get_cog(
                        "CronTasks"
                    )
            ...
            await interaction.response.send_message(
                embed=original_shown_embed, view=None, content=None
            )
    ...
    async def sync(
        ...
        if channel is None:
            ...
            await interaction.response.send_message(
                f"Permissions for {channel.mention} synced with {channel.category}"
            )
    ...
    async def kick(
        ...
        await interaction.response.send_message(
            f"Are you sure you want to kick `{member}` for `{reason}`", view=view
        )
        await view.wait()
        ...
        if view.value is True:
            ...
            if reason is None:
                await interaction.response.send_message(
                    "Please specify a reason why you want to kick this user!"
                )
    ...
    async def mute(
        ...
        message = (
            f"You have been muted from the TMS Scioly Discord server for {reason}."
        )
    ...
    async def nuke(self, interaction: discord.Interaction, count: int):
        ...
        if int(count) > max_delete:
            return await interaction.response.send_message(
                "Chill. No more than deleting 100 messages at a time."
            )
    ...
    async def lock(
        ...
        if channel is None:
            ...
            await interaction.channel.set_permissions(
                SL, add_reactions=True, send_messages=True, read_messages=True
            )
            await interaction.response.send_message(
                f"Locked :lock: {interaction.channel.mention} to Member access."
            )
        else:
            member_role = interaction.guild.get_role(Role.M)
            ...
            await interaction.response.send_message(
                f"Locked :lock: {channel.mention} to Member access."
            )
    ...
    @checks.has_any_role(Role.SERVERLEADER, Role.FORMER_SL)
    @describe(
        reason="The reason you are warning this user", member="The user you are warning"
    )
    async def warn(
        self, interaction: discord.Interaction, member: discord.Member, reason: str
    ):
        ...
        if member is interaction.user:
            return await interaction.response.send_message(
                "You cannot warn yourself :rolling_eyes:"
            )
        if member.id in TMS_BOT_IDS:
            return await interaction.response.send_message(
        ...
        embed.set_author(name=f"{member}", icon_url=avatar1)

        embed1 = discord.Embed(
            title=" ", description=f"{member} has been warned", color=0x2E66B6
        )
        ...
        embed2.set_author(name=f"{mod}", icon_url=avatar)
    ...
    async def blacklist(self, interaction: discord.Interaction, member: discord.Member):
        ...
        if member.id == self.bot.owner_id:
            return await interaction.response.send_message(
                "You can't blacklist the owner of the bot :rolling_eyes:"
            )
        try:
            ...
            if member.id in data["blacklisted_ids"]:
                ...
                with open("blacklist.json", "w") as f:
                    json.dump(data, f)
                await interaction.response.send_message(
                    f"Blacklisted {member.mention} from using commands!"
                )
        except Exception:
            await interaction.response.send_message(
                f"Failed to blacklist {member.mention}!"
            )
    ...
    async def unblacklist(
        ...
        if member.id in data["blacklisted_ids"]:
            ...
            with open("blacklist.json", "w") as f:
                ...
                await interaction.response.send_message(
                    f"Successfully removed command blacklist from {member.mention}"
                )
            #
            ...
            await interaction.response.send_message(
                f"{member.mention} is not blacklisted from using commands"
            )
