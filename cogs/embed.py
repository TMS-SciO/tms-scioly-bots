...
class EmbedFieldManagerView(discord.ui.View):
    ...
    def __init__(self, interaction: discord.Interaction, fields, index):
        ...
        self.add_item(
            EmbedFieldManagerButton(self, f"Inline: {self.field['inline']} (Toggle)", "inline", status="toggle"))
...
class EmbedButton(discord.ui.Button["EmbedView"]):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.update_value in ['author_icon', 'author_url'] and not any(
                [value in self.embed_view.embed_dict for value in ['author_name', 'authorName']]):
        ...
        if self.update_value == 'add_field':
            ...
            self.embed_view.embed_update['add_field'] = {
                'index': len(self.embed_view.embed_dict['fields']) if 'fields' in self.embed_view.embed_dict else 0}
...
class EmbedView(discord.ui.View):
    ...
    def __init__(self, embed_dict: Dict, interaction: discord.Interaction):
        ...
        associations = [
            ...
            {'proper_name': 'Thumbnail Image (from URL)', 'dict_values': ['thumbnail_url', 'thumbnailUrl'], 'row': 1,
             'help': "Please note that only HTTPS URLs will work. To remove the thumbnail, respond simply with `remove`."},
            {'proper_name': 'Image (from URL)', 'dict_values': ['image_url', 'imageUrl'], 'row': 1,
             'help': "Please note that only HTTPS URLs will work. To remove the image, simply respond with `remove`."},
            {'proper_name': 'Author Name', 'dict_values': ['author_name', 'authorName'], 'row': 2,
             'help': "To remove the author name (and therefore, the author icon/URL), simply respond with `remove`."},
            {'proper_name': 'Author Icon (from URL)', 'dict_values': ['author_icon', 'authorIcon'], 'row': 2,
             'help': "To remove the author icon, simply respond with `remove`."},
            {'proper_name': 'Author URL', 'dict_values': ['author_url', 'authorUrl'], 'row': 2,
             'help': "To remove the URL link from the author value, simply respond with `remove`."},
            {'proper_name': 'Footer Text', 'dict_values': ['footer_text', 'footerText'], 'row': 2,
             'help': "To remove the footer text, simply respond with `remove`."},
            {'proper_name': 'Footer Icon (from URL)', 'dict_values': ['footer_icon', 'footerIcon'], 'row': 2,
        ...
        for association in associations:
            if len([dict_value for dict_value in association['dict_values'] if dict_value in embed_dict]):
                button = EmbedButton(self, f"Edit {association['proper_name']}", discord.ButtonStyle.gray,
                                     association['row'], association['dict_values'][0],
                                     association['help'] if 'help' in association else "")
                self.add_item(button)
            else:
                button = EmbedButton(self, f"Set {association['proper_name']}", discord.ButtonStyle.green,
                                     association['row'], association['dict_values'][0],
                                     association['help'] if 'help' in association else "")
                self.add_item(button)
        ...
        while not complete:
            ...
            if embed_field_manager:
                if 'fields' not in embed_dict:
                    embed_dict['fields'] = []
                view = EmbedFieldManagerView(interaction, embed_dict['fields'], embed_field_index)
            else:
                view = EmbedView(embed_dict, interaction)
            await interaction.edit_original_message(
                content=f"This embed will be sent to {channel.mention}:",
                embed=response, view=view)
            await view.wait()
            if view.stopped_status == None:
                if isinstance(view, EmbedFieldManagerView):
                    ...
                    if 'import' in view.embed_update:
                        ...
                        if not len(file_message.attachments) or file_message.attachments[
                    ...
                    if 'export' in view.embed_update:
                        ...
                        file_message = await interaction.channel.send(file=discord.File('embed_export.json'))
                        await asyncio.sleep(15)
                        await file_message.delete()

                    removed = False
                    easy_removes = ['title', 'url', 'color', 'thumbnail_url', 'image_url', 'footer_text', 'footer_url',
                                    'author_url', 'author_icon']
                    ...
                    if not removed and not any(
                            key in view.embed_update for key in ['add_field', 'edit_field', 'import', 'export']):
