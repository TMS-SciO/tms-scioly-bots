...
# CREDIT - https://github.com/aikaterna/aikaterna-cogs/tree/v3/discordexperiments
...
class Activities(commands.Cog):
    ...
    async def _create_invite(self, ctx, app_id: int, max_age: int, app_name: str):
        ...
        if not voice:
            return await ctx.respond("You have to be in a voice channel to use this command.")
        if not voice.channel.permissions_for(ctx.me).create_instant_invite is True:
